...
export function appendScripts(scriptsObject, workingDir) {
    const projectPackageFilePath = path.join(workingDir, 'package.json');
	return fileManager.readJson(projectPackageFilePath)
		.then(packageConfig => {
			packageConfig.scripts = Object.assign({}, packageConfig.scripts, scriptsObject);
